...
def format_record_fields(record):
    ...
    record["Phone"] = speech_constants.PHONE_PREPARED_STRING.format(record["Phone"]) \
        if record["Phone"].strip() != "" else speech_constants.NO_PHONE
    record["Fee"] = speech_constants.FEE_PREPARED_STRING.format(record["Fee"]) \
...
def get_snow_emergency_parking_intent(mycity_request):
    ...
    if intent_constants.CURRENT_ADDRESS_KEY in mycity_request.session_attributes:
        finder = FinderCSV(mycity_request, PARKING_INFO_URL, ADDRESS_KEY,
                           speech_constants.OUTPUT_SPEECH_FORMAT, format_record_fields)
        ...
        mycity_response.output_speech = speech_constants.ERROR_SPEECH
