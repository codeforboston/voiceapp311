...
MERCATOR = 'epsg:3857'  # defines pseudo-mercator coordinate system,
                        # i.e., it uses X,Y instead of lat,long
                        # (ref.: https://epsg.io/3857)
GEODETIC = 'epsg:4326'  # defines geodetic coordinate system,
                        # i.e., lat, long. (ref.: https://epsg.io/4326)
...
def get_truck_locations():
    ...
    food_truck_data_result = \
        gis_utils.get_features_from_feature_server(FOOD_TRUCK_FEATURE_SERVER_URL,
                                                   FOOD_TRUCK_QUERY)
    ...
    for truck in food_truck_data_result:
        if truck["attributes"]["Loc"] not in truck_unique_locations:
            truck_unique_locations.append([truck["attributes"]["Loc"],
                                           truck["geometry"]])
        ...
        try:
            ...
            if count == 1:
                mycity_response.output_speech = \
                    "I found {} food truck within a mile " \
                    "from your address.".format(count)
            elif count == 2:
                mycity_response.output_speech = \
                    "I found {} food trucks within a mile " \
                    "from your address.".format(count)
            else:
                mycity_response.output_speech = \
                    "I found {} food trucks within a mile " \
